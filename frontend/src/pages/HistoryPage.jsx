  ...
  return (
    <PageTransition className="page-shell pb-24 pt-8">
      <div className="max-w-3xl">
        <p className="text-sm uppercase tracking-[0.28em] text-[#A96C00]">Fruit civilizations</p>
      ...
      <div className="glass mt-8 flex max-w-2xl items-center gap-3 rounded-full p-2">
        <Search className="ml-4 text-[#7C8579]" size={21} />
        <input
          value={query}
          onChange={(event) => setQuery(event.target.value)}
          placeholder="Search fruit history"
          className="min-w-0 flex-1 bg-transparent py-3 text-[#141814] outline-none placeholder:text-[#8C9589]"
        />
      </div>
            ...
            <div className="p-7">
              <div className="grid gap-4 md:grid-cols-3">
                <div className="rounded-3xl bg-[#FAF9F5] p-5">
                  ...
                  <p className="mt-3 text-sm text-[#656E62]">Origin</p>
                  <p className="mt-1 font-semibold">{fruit.origin}</p>
                </div>
                <div className="rounded-3xl bg-[#FAF9F5] p-5 md:col-span-2">
                  <Landmark className="text-amber-200" />
                  <p className="mt-3 text-sm text-[#656E62]">Cultural significance</p>
                  <p className="mt-1 leading-7 text-[#141814]">{fruit.culture}</p>
                </div>
              </div>
              <div className="mt-6 grid gap-4">
                {fruit.timeline?.map((event) => (
                  <div key={`${fruit.slug}-${event.year}`} className="flex gap-4 rounded-3xl border border-[#E8E6DD] bg-[#FAF9F5] p-4">
                    <Clock3 className="mt-1 shrink-0 text-[#656E62]" size={18} />
                    <div>
                      <p className="font-bold" style={{ color: fruit.color }}>{event.year}</p>
                      <p className="mt-1 text-[#141814]">{event.event}</p>
                    </div>
                  </div>
                ))}
              </div>
