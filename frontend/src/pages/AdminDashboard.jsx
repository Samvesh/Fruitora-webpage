    ...
    return (
      <PageTransition className="page-shell pb-24 pt-8">
        <div className="glass rounded-[2rem] p-8">
          <p className="text-sm uppercase tracking-[0.28em] text-[#A96C00]">Admin analytics</p>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        {stats.map(({ label, value, icon: Icon }) => (
          <motion.div key={label} whileHover={{ y: -7 }} className="glass rounded-[2rem] p-6">
            ...
            <p className="mt-1 text-sm text-[#656E62]">{label}</p>
          </motion.div>
        ))}
      </div>
        ...
        <section className="glass rounded-[2rem] p-6">
          ...
          <div className="mt-5 grid gap-3">
            {analytics.searches.length ? analytics.searches.map((item, index) => (
              <div key={item._id} className="grid grid-cols-[32px_1fr_auto] items-center gap-3 rounded-2xl bg-[#FAF9F5] p-4">
                <span className="text-[#656E62]">{index + 1}</span>
                <span>{item._id}</span>
                <span className="rounded-full bg-white px-3 py-1 text-sm font-bold text-[#141814]">{item.count}</span>
              </div>
            )) : <p className="rounded-2xl bg-[#FAF9F5] p-4 text-[#556453]">No verified live data available</p>}
          </div>
        </section>
        ...
        <section className="glass rounded-[2rem] p-6">
          ...
          <div className="mt-5 grid gap-3">
            {analytics.usersByRegion.length ? analytics.usersByRegion.map((item) => (
              <div key={item._id} className="rounded-2xl bg-[#FAF9F5] p-4">
                <div className="mb-2 flex items-center justify-between">
                  <span className="flex items-center gap-2"><Globe2 size={16} /> {item._id}</span>
                  <span>{item.users}</span>
                </div>
                <div className="h-2 rounded-full bg-[#E2E0D5]">
        ...
        <section className="glass rounded-[2rem] p-6 lg:col-span-2">
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-4">
            {analytics.healthTrends.length ? analytics.healthTrends.map((trend) => (
              <div key={trend.label} className="rounded-3xl bg-[#FAF9F5] p-5">
                <p className="text-sm text-[#656E62]">{trend.label}</p>
                <p className="mt-3 text-4xl font-black">{trend.value}%</p>
              </div>
            )) : <p className="rounded-2xl bg-[#FAF9F5] p-4 text-[#556453] md:col-span-4">{analytics.note || "No health trend events have been captured yet."}</p>}
          </div>
        </section>
      </div>
    </PageTransition>
